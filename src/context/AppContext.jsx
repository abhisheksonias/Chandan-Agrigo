...
import { transportService } from '@/lib/supabaseService';
...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load transports from Supabase
        const { success, data, error } = await transportService.getAllTransports();
        ...
        if (success && data) {
          ...
          toast({ 
            ...
            description: 'Failed to load transport data from the server', 
            variant: 'destructive' 
          });
  ...
  const addTransport = async (transportData) => {
    try {
      // Save to Supabase
      const { success, data, error } = await transportService.addTransport(transportData);
      
      if (success && data) {
        // Update local state with the returned data (including the ID generated by Supabase)
        setTransports(prev => [...prev, data]);
        toast({ title: 'Transport Added', description: `${data.name} has been added.` });
        return data;
      } else {
        throw new Error(error || 'Failed to add transport');
      }
    } catch (error) {
  ...
  // Add functions to update and delete transports
  const updateTransport = async (transportId, transportData) => {
    try {
      const { success, data, error } = await transportService.updateTransport(transportId, transportData);
      
      if (success && data) {
        // Update local state with the returned data
        setTransports(prev => prev.map(transport => 
          transport.id === transportId ? data : transport
        ));
        toast({ title: 'Transport Updated', description: `${data.name} has been updated.` });
        return data;
      } else {
        throw new Error(error || 'Failed to update transport');
      }
    } catch (error) {
  ...
  const deleteTransport = async (transportId) => {
    try {
      const { success, error } = await transportService.deleteTransport(transportId);
      
      if (success) {
        // Update local state by removing the deleted transport
        ...
        throw new Error(error || 'Failed to delete transport');
      }
    } catch (error) {
